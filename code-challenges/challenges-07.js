...
const objLat = (obj) => {
    ...
    // method to upper casses for the first letter from https://flexiple.com/javascript/javascript-capitalize-first-letter/
};
// -------------------------------------------------------------------------------------------------------
...
const cvFormatter = (arr) => {
    ...
    for(let i=0; i<arr.length ; i++){
        let obj=arr[i];
        let obj2={};
        if(!(obj.lastName== null)&&obj.yearsOfExperience >1){
            obj2.fullName= `${obj.firstName} ${obj.lastName}`;
            obj2.tech=obj.tech ;
            outPut.push(obj2);
        } else if (obj.yearsOfExperience >1){
            obj2.fullName=`${obj.firstName}`;
            obj2.tech=obj.tech ;
            outPut.push(obj2);
        }
    }
...
const applicationsStatics = (arr) => {
    ...
    for(let i=0;i<arr.length ; i++){
        let obj=arr[i];
        if(!(obj.lastName== null&&obj.firstName==null)&&obj.yearsOfExperience >1){
            switch (obj.tech){
                case "JS":
                    javascript +=1 ;
                    break ;
...
const avg=(arr)=>{
    ...
    avg=(sum/arr.length) ;
    // return Math.round(avg) ; //this not work with test its to floor number not round
...
const classesAvg = (data) => {
    ...
    let arr=data["grades"] ;
    for (let i=0;i<arr.length ;i++){
        let arr1=arr[i]["classes"] ;
        for(let r=0 ;r<arr1.length ; r++){
            let arr2=arr1[r]["classScores"];
            arr1[r]["avg"]=avg(arr2);     
            // console.log(avg(arr2))    // correct
        }
    }
    return data ;
};
